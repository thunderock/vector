...
---
*Last updated: 2026-05-27 after Phase 9.1 verified — five prior-phase user-visible gaps closed end-to-end so Phase 9 reconnect/persist features are reachable and v1.0.0 surface is polished. (1) Local pane `exit` now closes pane/tab/window/app via `Mux::close_pane` cascade in the `PaneExited` handler (Phase 4 stub retired). (2) `vector-codespaces` crate deleted wholesale; GitHub OAuth, Codespaces UI, `auth_actor`/`codespaces_actor`/`codespaces_modal` modules, 10 Phase-6 UserEvent variants, and 2 AppShortcut variants removed — Microsoft is now the only v1 sign-in path. (3) `install_microsoft_menu_items(mtm, proxy)` wired into `App::resumed` so `Sign in/out of Microsoft` and `Dev Tunnels…` items actually appear in the menu at startup. (4) `DevTunnelsPickerModal` gained an `NSButton` "Sign in with Microsoft" in `NotSignedIn` state that fires the same actor command as the menu item; picker auto-refreshes on `MicrosoftSignedIn`; footer copy stripped of "GitHub or" at all five sites. (5) Typographic V app icon authored, `xtask icon` subcommand wired (latent Phase 1 bug), `CFBundleIconFile=icon` set in `Info.plist.partial` — bundled `.app` shows the Vector glyph in dock/Finder/Cmd-Tab. Workspace `cargo test --workspace` 478 suites passing / 0 failed; `cargo check --workspace` clean. Parent Phase 9 UAT's five gaps marked `status: resolved`; tests 4–9 still blocked pending user re-run of `/gsd:verify-work 9` against a fresh build. Phase 10 (hardening + release) is next — 10-04 (release tag) was held pending this closure.*
